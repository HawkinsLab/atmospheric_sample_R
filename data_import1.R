...
####################################
# Get the file names in the pattern specified
# All the raw data files have a .txt extension.
files <- list.files(pattern="*.txt")
#txt0 <- strsplit(files[1], split = "_")
#txt_array = txt0[[1]][1]
raw_data_array <- list() # a place holder (an empty list obj)
# use cbind (column bind) function to make a 
for(file in files)
{
  ...
  raw_data_array <- cbind(raw_data_array, readLines(file))
}
